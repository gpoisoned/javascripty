 ...
 def reverse[T] (l:List[T]): List[T] = 
  ...
  def typeInfer(env: Map[String,Typ], e: Expr): Typ = {
    ...
    e match {
      ...
      case Binary(Eq|Ne, e1, e2) => (typ(e1),typ(e2)) match {
        case (x,y) => if (!hasFunctionTyp(x) && !hasFunctionTyp(y) && (x ==y)) TBool else err(x,e1)
      }
      ...
      case Function(p, params, tann, e1) => {
        ...
        tann match {
          ...
          case Some(tret) => {
            if(tret == typeInfer(env2,e1)) TFunction(params, tret) else TFunction(params,err(tret,e1))                
          }
        }
      }
      case Call(e1, args) => typ(e1) match {
        case TFunction(params, tret) if (params.length == args.length) => {
          (params, args).zipped.foreach {
            case (x,y) => x match{
              case (a,b) => if (b != typ(y)) err(typ(y),y) 
            } 
          };
          tret
        }
      ...
      case Obj(fields) =>{
       TObj(fields.mapValues(a=>typ(a)))
     //  TObj(fields.foldLeft(Map[String,Typ]())((acc,a) => a match {case (c,d) => acc + (c -> typ(d)) } ))
      }
  ...
  def substitute(e: Expr, v: Expr, x: String): Expr = {
    ...
    e match {
      ...
      case GetField(e1, f) =>{
        if (x != f) GetField(subst(e1), f) else e
      }
  ...
  def step(e: Expr): Expr = {
    ...
    e match {
      ...
      case If(e1,e2,e3) if isValue(e1) => if(e1 == B(true)) e2 else e3
        ...
        v1 match {
          case Function(p, params, _, e1) => {
            val e1p = (params, args).zipped.foldRight(e1){
             (p,e1new) => p match{
               case((par,_),v) => substitute(e1new,v,par)
             }
            }
            p match {
              case None => e1p
      ...
      /*** Fill-in more cases here. ***/
      case Call(Function(p, params, tann,e1), args) => {
       Call(Function(p,params,tann,e1), args.foldRight(List[Expr]())((a,acc) => if (!isValue(a))step(a)::acc  else a::acc)  )
      }  
